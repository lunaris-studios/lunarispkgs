# Make sure that the "withGce" flag is set when building `google-cloud-sdk`
...
let
  pythonEnv = python.withPackages (p: with p; [
    ...
    pyopenssl
    crcmod
  ] ++ lib.optional (withGce) google-compute-engine);
...
in stdenv.mkDerivation rec {
  ...
  patches = [
    ./patch/gcloud-path.patch
    ./patch/gsutil-disable-updates.patch
  ];
  ...
  installPhase = ''
    ...
    cp ${./script/alpha__init__.py} $out/google-cloud-sdk/lib/surface/alpha/__init__.py
    cp ${./script/beta__init__.py} $out/google-cloud-sdk/lib/surface/beta/__init__.py
